...
var _srcPhotoPicker = require('../src/PhotoPicker');

var _srcPhotoPicker2 = _interopRequireDefault(_srcPhotoPicker);

var _srcFacebookPhotoProvider = require('../src/FacebookPhotoProvider');

var _srcFacebookPhotoProvider2 = _interopRequireDefault(_srcFacebookPhotoProvider);

var _srcInstagramPhotoProvider = require('../src/InstagramPhotoProvider');

var _srcInstagramPhotoProvider2 = _interopRequireDefault(_srcInstagramPhotoProvider);
...
(0, _jquery2['default'])(function () {
  ...
  (0, _jquery2['default'])('.no-js').removeClass('no-js').addClass('js');

  // Handle selected photo
  (0, _jquery2['default'])(document.documentElement).on('selected.photopicker', function (e, photo, provider) {
    if (photo) {

      _srcUtil.debug.log(photo, provider);

      var img = new Image();

      img.src = photo.getSrc();

      (0, _jquery2['default'])('#preview').empty().append(img);
    }
  ...
  // Lightbox demo
  (0, _jquery2['default'])('#lightbox-demo').lightbox('.default-lightbox', {
    descendantSelector: '.item'
  });

  var facebookPhotoProvider = new _srcFacebookPhotoProvider2['default']({
    appId: facebookAppIDs[window.location.host],
    status: true,
    xfbml: false
  }),
      instagramPhotoProvider = new _srcInstagramPhotoProvider2['default'](instagramClient[window.location.host].id, instagramClient[window.location.host].url),
      facebookPicker = new _srcPhotoPicker2['default']('#photo-picker', facebookPhotoProvider, { lightbox: { title: 'Facebook Photos', extraClass: 'facebook-picker-lightbox', titleSelector: '.lightbox-title-text' } }),
      instagramPicker = new _srcPhotoPicker2['default']('#photo-picker', instagramPhotoProvider, { lightbox: { title: 'Instagram Photos', extraClass: 'instagram-picker-lightbox', titleSelector: '.lightbox-title-text' } });

  (0, _jquery2['default'])('#facebook-picker-button').on('click', _jquery2['default'].proxy(facebookPicker.open, facebookPicker));
  (0, _jquery2['default'])('#instagram-picker-button').on('click', _jquery2['default'].proxy(instagramPicker.open, instagramPicker));
});

},{"../src/FacebookPhotoProvider":101,"../src/InstagramPhotoProvider":102,"../src/Lightbox":103,"../src/PhotoPicker":105,"../src/util":108,"babelify/polyfill":94,"jquery":96}],2:[function(require,module,exports){
(function (global){
"use strict";
...
(function(global,factory){if(typeof define === 'function' && define.amd){define(['exports','module'],factory);}else if(typeof exports !== 'undefined' && typeof module !== 'undefined'){factory(exports,module);}else {var mod={exports:{}};factory(mod.exports,mod);global.popup = mod.exports;}})(this,function(exports,module){'use strict';var _createClass=(function(){function defineProperties(target,props){for(var i=0;i < props.length;i++) {var descriptor=props[i];descriptor.enumerable = descriptor.enumerable || false;descriptor.configurable = true;if('value' in descriptor)descriptor.writable = true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};})();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError('Cannot call a class as a function');}}var Popup=(function(){function Popup(){var url=arguments.length <= 0 || arguments[0] === undefined?'':arguments[0];var features=arguments.length <= 1 || arguments[1] === undefined?{}:arguments[1];_classCallCheck(this,Popup);this.url = url || '';this.name = '_blank';this.win = null;this.timer = null;this.blockedCallback = null;this.closedCallback = null;this.openedCallback = null;this.setFeatures(features);}_createClass(Popup,[{key:'setFeatures',value:function setFeatures(features){this.features = Popup.features;if(features.name){this.name = features.name;delete features.name;}for(var f in features) {if(!(f in this.features)){continue;}if(typeof features[f] === 'boolean'){this.features[f] = features[f]?1:0;}else {this.features[f] = features[f];}}if(this.features.width === null){this.features.width = window.screen.availWidth / 2;}if(this.features.height === null){this.features.height = window.screen.availHeight / 2;}if(this.features.left === null){this.features.left = (window.screen.availWidth - this.features.width) / 2;}if(this.features.top === null){this.features.top = (window.screen.availHeight - this.features.height) / 2;}this.features.width = this.features.width | 0;this.features.height = this.features.height | 0;this.features.left = this.features.left + window.screen.availLeft | 0;this.features.top = this.features.top + window.screen.availTop | 0;return this;}},{key:'featuresString',value:function featuresString(){var features=[];for(var f in this.features) {if(this.features.hasOwnProperty(f) && this.features[f] !== null){features[features.length] = f + '=' + this.features[f];}}return features.join(',');}},{key:'open',value:function open(){this.win = window.open(this.url,this.name,this.featuresString());if(this.win && !this.win.closed){this.win.moveTo(this.features.left,this.features.top);this.win.focus();if(typeof this.openedCallback === 'function'){this.openedCallback(this);}this.waitForClosed();}else if(typeof this.blockedCallback === 'function'){this.blockedCallback(this);}return this;}},{key:'close',value:function close(){if(this.win && !this.win.closed){this.win.close();}return this;}},{key:'clearTimer',value:function clearTimer(){window.clearInterval(this.timer);this.timer = null;}},{key:'waitForClosed',value:function waitForClosed(){var _this=this;if(typeof this.closedCallback === 'function'){this.clearTimer();this.timer = window.setInterval(function(){if(!_this.win || _this.win.closed){_this.clearTimer();_this.closedCallback(_this);}},500);}}},{key:'setCallback',value:function setCallback(name,callback){var cb=name + 'Callback';if(typeof callback === 'function' && this.hasOwnProperty(cb)){this[cb] = callback;}return this;}},{key:'blocked',value:function blocked(callback){return this.setCallback('blocked',callback);}},{key:'opened',value:function opened(callback){return this.setCallback('opened',callback);}},{key:'closed',value:function closed(callback){return this.setCallback('closed',callback);}}]);return Popup;})();Popup.features = {width:null,height:null,left:null,top:null,menubar:0,toolbar:0,location:1,status:1,resizable:1,scrollbars:1};module.exports = Popup;});
...
var _ContentProvider2 = require('./ContentProvider');

var _ContentProvider3 = _interopRequireDefault(_ContentProvider2);

var AjaxContentProvider = (function (_ContentProvider) {
  _inherits(AjaxContentProvider, _ContentProvider);

  function AjaxContentProvider() {
    var url = arguments.length <= 0 || arguments[0] === undefined ? '' : arguments[0];

    var _ref = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];

    var _ref$alwaysFetch = _ref.alwaysFetch;
    var alwaysFetch = _ref$alwaysFetch === undefined ? false : _ref$alwaysFetch;
    var _ref$ajaxOptions = _ref.ajaxOptions;
    var ajaxOptions = _ref$ajaxOptions === undefined ? {} : _ref$ajaxOptions;
    var _ref$processData = _ref.processData;
    var processData = _ref$processData === undefined ? function (data) {
      return data;
    } : _ref$processData;

    _classCallCheck(this, AjaxContentProvider);

    _get(Object.getPrototypeOf(AjaxContentProvider.prototype), 'constructor', this).call(this, '', { alwaysFetch: alwaysFetch, ajaxOptions: ajaxOptions, processData: processData });

    this.url = url;
  }

  _createClass(AjaxContentProvider, [{
    key: 'fetchContent',
    value: function fetchContent() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _jquery2['default'].ajax(_this.url, _this.options.ajaxOptions).done(function (data, textStatus, jqXHR) {

          if (_this.options.processData) {

            _this.setContent(_this.options.processData(data, textStatus, jqXHR));
          } else {
    ...
    value: function getContent() {
      ...
      return _get(Object.getPrototypeOf(AjaxContentProvider.prototype), 'getContent', this).call(this);
    }
  }]);
...
exports['default'] = AjaxContentProvider;
module.exports = exports['default'];

},{"./ContentProvider":100,"jquery":96}],99:[function(require,module,exports){
'use strict';
...
var Album = (function () {
  function Album(id, name) {
    var photosURL = arguments.length <= 2 || arguments[2] === undefined ? '' : arguments[2];
    var coverPhoto = arguments.length <= 3 || arguments[3] === undefined ? '' : arguments[3];
  ...
  _createClass(Album, [{
    key: 'addPhotoID',
    value: function addPhotoID(id) {
      this.photoIDs.push(id);
      return this;
    }
  }, {
    key: 'getPhotoIDs',
    value: function getPhotoIDs() {
    ...
    value: function getHTML() {
      ...
      if (this.photo instanceof _Photo2['default']) {
...
},{"./Photo":104}],100:[function(require,module,exports){
'use strict';
...
var _PhotoProvider2 = require('./PhotoProvider');

var _PhotoProvider3 = _interopRequireDefault(_PhotoProvider2);
...
var FacebookPhotoProvider = (function (_PhotoProvider) {
  _inherits(FacebookPhotoProvider, _PhotoProvider);

  function FacebookPhotoProvider() {
    var sdkSettings = arguments.length <= 0 || arguments[0] === undefined ? {} : arguments[0];

    _classCallCheck(this, FacebookPhotoProvider);

    _get(Object.getPrototypeOf(FacebookPhotoProvider.prototype), 'constructor', this).call(this, '/me/photos?type=uploaded&fields=id,images,from', '/me/albums?fields=id,name,count,cover_photo{id,images,picture,source},description,type,privacy');
  ...
  _createClass(FacebookPhotoProvider, [{
    key: 'login',
    value: function login() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        (0, _facebookSdk2['default'])(_this.sdkSettings).then(function (FB) {
          ...
          FB.login(function (response) {
            ...
            if (response.authResponse) {
              ...
              if (response.authResponse.grantedScopes) {
                ...
                var ok = _this.scopes.every(function (scope) {
    ...
    value: function init() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        (0, _facebookSdk2['default'])(_this2.sdkSettings).then(function (FB) {

          FB.getLoginStatus(function (response) {

            if (response.status === 'connected') {

              _this2.api('/me/permissions', {}).then(function (res) {

                res.data.every(function (d) {
                  if (d.status !== 'granted') {
    ...
    value: function api(url) {
      ...
      parameters = _jquery2['default'].extend(this.getParameters(), parameters);
      ...
      return new Promise(function (resolve, reject) {
        ...
        (0, _facebookSdk2['default'])(_this3.sdkSettings).then(function (FB) {

          FB.api(url, parameters, function (response) {

            if (response && !response.error) {

              _util.debug.log('FB api response', response);
              resolve(response);
            } else {

              var error = new Error(response.error.message ? response.error.message : 'FB.api failed');

              _util.debug.error('FB api error', error);
              reject(error);
            }
          });
        },
    ...
    key: 'apiGetPhotoURL',
    value: function apiGetPhotoURL(photoId) {
      return '/' + photoId + '?fields=id,images,picture,source,from';
    }
  }, {
    key: 'apiGetAlbumURL',
    value: function apiGetAlbumURL(albumId) {
    ...
    key: 'apiGetAlbumPhotosURL',
    value: function apiGetAlbumPhotosURL(albumId) {
      return '/' + albumId + '/photos?fields=id,images,from';
    }
  }, {
    key: 'getNextUrl',
    value: function getNextUrl(results) {
      return results.paging && results.paging.next ? results.paging.next : false;
    }
  }, {
    key: 'buildPhoto',
    value: function buildPhoto(data) {
      ...
      return new _Photo2['default'](data.id, source, thumbnail);
    }
  }, {
    key: 'buildAlbum',
    value: function buildAlbum(data) {
      _util.debug.log(data);

      var album = new _Album2['default'](data.id, data.name, this.apiGetAlbumPhotosURL(data.id));

      if (typeof data.cover_photo === 'object' && data.cover_photo.id) {
        var photo = this.buildPhoto(data.cover_photo);
        this.addPhoto(photo);
        album.photo = photo;
...
})(_PhotoProvider3['default']);

exports['default'] = FacebookPhotoProvider;
module.exports = exports['default'];

},{"./Album":99,"./Photo":104,"./PhotoProvider":106,"./facebook-sdk":107,"./util":108,"jquery":96}],102:[function(require,module,exports){
'use strict';
...
var _popupWindow = require('popup-window');

var _popupWindow2 = _interopRequireDefault(_popupWindow);

var InstagramPhotoProvider = (function (_PhotoProvider) {
  _inherits(InstagramPhotoProvider, _PhotoProvider);

  function InstagramPhotoProvider(clientID, redirectURI) {
    _classCallCheck(this, InstagramPhotoProvider);

    _get(Object.getPrototypeOf(InstagramPhotoProvider.prototype), 'constructor', this).call(this, 'https://api.instagram.com/v1/users/self/media/recent', null);
  ...
  _createClass(InstagramPhotoProvider, [{
    key: 'init',
    value: function init() {
      var _this = this;
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (_this.accessToken !== null) {
          ...
          if (!_this.clientID || !_this.redirectURI) {

            reject(new Error('Please provide clientID and redirectURI'));
          } else {

            var closedOrBlocked = function closedOrBlocked() {
            ...
            new _popupWindow2['default'](_this.authorizeURL, {
              name: 'instagram',
              width: 650,
              height: 480
            }).opened(function () {
    ...
    key: 'buildPhoto',
    value: function buildPhoto(data) {
      var photo = new _Photo2['default'](data.id, data.images.standard_resolution.url.replace('http://', '//'), data.images.thumbnail.url.replace('http://', '//'), data.likes.count, data.tags);
...
exports['default'] = InstagramPhotoProvider;
module.exports = exports['default'];

},{"./Photo":104,"./PhotoProvider":106,"jquery":96,"popup-window":97}],103:[function(require,module,exports){
'use strict';
...
var _AjaxContentProvider = require('./AjaxContentProvider');

var _AjaxContentProvider2 = _interopRequireDefault(_AjaxContentProvider);

var Lightbox = (function () {
  function Lightbox(template, contentProvider) {
    var _ref = arguments.length <= 2 || arguments[2] === undefined ? {} : arguments[2];

    var _ref$closeOnESC = _ref.closeOnESC;
    var closeOnESC = _ref$closeOnESC === undefined ? true : _ref$closeOnESC;
    var _ref$htmlClass = _ref.htmlClass;
    var htmlClass = _ref$htmlClass === undefined ? 'lightbox-open' : _ref$htmlClass;
    var _ref$openClass = _ref.openClass;
    var openClass = _ref$openClass === undefined ? 'open' : _ref$openClass;
    var _ref$closeSelector = _ref.closeSelector;
    var closeSelector = _ref$closeSelector === undefined ? '.lightbox-close-action' : _ref$closeSelector;
    var _ref$titleSelector = _ref.titleSelector;
    var titleSelector = _ref$titleSelector === undefined ? '.lightbox-title' : _ref$titleSelector;
    var _ref$extraClass = _ref.extraClass;
    var extraClass = _ref$extraClass === undefined ? '' : _ref$extraClass;

    _classCallCheck(this, Lightbox);

    this.$template = (0, _jquery2['default'])(template);
  ...
  _createClass(Lightbox, [{
    key: 'trigger',
    value: function trigger(name, parameters) {
      (0, _jquery2['default'])(document.documentElement).trigger(name, parameters);
    }
  }, {
    key: 'setContentProvider',
    value: function setContentProvider() {
      var contentProvider = arguments.length <= 0 || arguments[0] === undefined ? null : arguments[0];

      this.contentProvider = contentProvider instanceof _ContentProvider2['default'] ? contentProvider : new _ContentProvider2['default']();
    }
  }, {
    key: 'renderContent',
    value: function renderContent() {
      ...
      this.contentProvider.getContent().then(function (content) {
        $contentFrame.append(content);
        _this.contentProvider.bindEvents();
        _this.$template.removeClass('loading');
        _this.trigger('render.lightbox', [_this]);
      }, function (error) {
        console.error(error);
      });
    }
    ...
    value: function addClasses() {
      (0, _jquery2['default'])(document.documentElement).addClass(this.option('htmlClass'));
    ...
    value: function removeClasses() {
      (0, _jquery2['default'])(document.documentElement).removeClass(this.option('htmlClass'));
    ...
    value: function close(event) {
      if (event) {
        ...
        if (!(0, _jquery2['default'])(event.currentTarget).data('allow-default')) {
          event.preventDefault();
          event.stopPropagation();
        }
      }
    ...
    key: 'bindEvents',
    value: function bindEvents() {
      (0, _jquery2['default'])(document.documentElement).on('keyup.lightbox', _jquery2['default'].proxy(this.checkESC, this));
      this.$template.on('click.lightbox', this.option('closeSelector'), _jquery2['default'].proxy(this.close, this));
      return this;
    }
  }, {
    key: 'unbindEvents',
    value: function unbindEvents() {
      (0, _jquery2['default'])(document.documentElement).off('keyup.lightbox');
...
_jquery2['default'].fn.lightbox = function (options) {
  var settings = _jquery2['default'].extend({}, _jquery2['default'].fn.lightbox.defaults, options);
  ...
  function handleClick(event) {
    ...
    switch (true) {
      case href !== false:
        provider = new _AjaxContentProvider2['default'](href, settings.provider);
        break;
      case content !== false:
        provider = new _ContentProvider2['default'](content);
        break;
      case template !== false:
    ...
    var lightbox = new Lightbox(settings.template, provider, {
      title: title,
...
_jquery2['default'].fn.lightbox.defaults = {
  template: '.default-lightbox',
  descendantSelector: null,
  provider: {
    ajaxOptions: {},
    processData: function processData(data, textStatus, jqXHR) {
...
},{"./AjaxContentProvider":98,"./ContentProvider":100,"jquery":96}],104:[function(require,module,exports){
'use strict';
...
var Photo = (function () {
  function Photo(id, src, thumbnail) {
  ...
  _createClass(Photo, [{
    ...
    value: function setTags(tags) {
      if (tags) {
        ...
        this.tags = [];
        (_tags = this.tags).push.apply(_tags, _toConsumableArray(tags));
      }
...
var _PhotoProvider = require('./PhotoProvider');

var _PhotoProvider2 = _interopRequireDefault(_PhotoProvider);

var PhotoPicker = (function (_ContentProvider) {
  _inherits(PhotoPicker, _ContentProvider);

  function PhotoPicker(templateSelector, photoProvider) {
    ...
    _get(Object.getPrototypeOf(PhotoPicker.prototype), 'constructor', this).call(this, '', options);
  ...
  _createClass(PhotoPicker, [{
    ...
    value: function init() {
      ...
      if (!this.setupLightbox) {
        this.setupLightbox = new Promise(function (resolve) {
          _this.setupTemplate().initLightbox();
          resolve(_this);
        });
      }
    ...
    key: 'numAlbums',
    value: function numAlbums() {
      return this.photoProvider.numAlbums();
    }
  }, {
    key: 'numPhotos',
    value: function numPhotos() {
    ...
    key: 'bindEvents',
    value: function bindEvents() {
      (0, _jquery2['default'])(document.documentElement).on('keyup.photopicker', _jquery2['default'].proxy(this.handleKeyup, this));

      if (this.photosPanel) {
        this.photosPanel.on('click.photopicker', '.photo', _jquery2['default'].proxy(this.handlePhotoClick, this));
        this.photosPanel.on('dblclick.photopicker', '.photo', _jquery2['default'].proxy(this.handlePhotoDoubleClick, this));
      }

      if (this.albumsPanel) {
        this.albumsPanel.on('click.photopicker', '.album', _jquery2['default'].proxy(this.handleAlbumClick, this));
      }

      if (this.photosButton) {
        this.photosButton.on('click.photopicker', _jquery2['default'].proxy(this.handlePhotosButtonClick, this));
      }

      if (this.albumsButton) {
        this.albumsButton.on('click.photopicker', _jquery2['default'].proxy(this.handleAlbumsButtonClick, this));
      }

      if (this.loadMoreButton) {
        this.loadMoreButton.on('click.photopicker', _jquery2['default'].proxy(this.handleLoadMoreButtonClick, this));
      }

      if (this.cancelButton) {
        this.cancelButton.on('click.photopicker', _jquery2['default'].proxy(this.handleCancelButtonClick, this));
      }

      if (this.submitButton) {
        this.submitButton.on('click.photopicker', _jquery2['default'].proxy(this.handleSubmitButtonClick, this));
      }
    ...
    key: 'unbindEvents',
    value: function unbindEvents() {
      (0, _jquery2['default'])(document.documentElement).off('keyup.photopicker');
    ...
    value: function initLightbox() {
      if (!this.lightbox) {
        this.lightbox = new _Lightbox2['default'](this.options.lightboxSelector, this, this.options.lightbox);
      }
    ...
    value: function open() {
      ...
      this.init().then(function (initResults) {
        var _initResults = _slicedToArray(initResults, 2);

        var picker = _initResults[0];
        var provider = _initResults[1];

        if (picker instanceof PhotoPicker && provider instanceof _PhotoProvider2['default']) {
          ...
          _util.debug.error('PhotoPicker and PhotoProvider not initialized properly');
        }
      }, function (error) {
        _this2.initialized = null; // clear out the Promise so that you don't get stuck in a rejected state.
        _this2.trigger('error.photopicker', [_this2, error]);
      });
    ...
    key: 'clearPhotosPanel',
    value: function clearPhotosPanel() {
      if (this.photosPanel) {
    ...
    value: function renderAlbums(albums) {
      ...
      if (!this.albumsRendered) {
        ...
        _util.debug.log(albums);

        (0, _util.objectToArray)(albums).forEach(function (album) {
          _this3.albumsPanel.append(album.getHTML());
        });
    ...
    key: 'hasAlbums',
    value: function hasAlbums() {
      return this.photoProvider.hasAlbums();
    }
  }, {
    key: 'toggleLoadingClass',
    value: function toggleLoadingClass(state) {
    ...
    value: function loadPhotos() {
      ...
      var cleanUp = function cleanUp() {
        _this4.toggleLoadingClass(false);
        _this4.toggleLoadMoreDisabled();
      };

      return this.photoProvider.loadPhotos().then(this.renderPhotos.bind(this)).then(cleanUp, cleanUp);
    }
  }, {
    key: 'toggleLoadMoreDisabled',
    value: function toggleLoadMoreDisabled(disabled) {
      if (disabled === void 0) {
    ...
    value: function loadMore() {
      ...
      var toggleButton = function toggleButton() {
        _this5.toggleLoadMoreDisabled();
      };

      this.loadPhotos()['catch'](_util.debug.warn).then(toggleButton, toggleButton);
    }
  }, {
    key: 'clearSelected',
    value: function clearSelected(selector) {
      this.selectedPhoto = null;
      var panel = selector ? (0, _jquery2['default'])(selector, this.content) : this.content;
      panel.find('.selected').removeClass('selected');
      this.submitButton.prop('disabled', 1);
    }
  }, {
    key: 'switchToPanel',
    value: function switchToPanel(panel) {
    ...
    key: 'switchToAlbum',
    value: function switchToAlbum(albumId) {
      var _this6 = this;
      ...
      if (albumId !== this.photoProvider.currentAlbumID) {
        ...
        this.clearPhotosPanel().getPhotos().then(function () {
          _this6.renderPhotos(_this6.photoProvider.getCurrentAlbumPhotos());
          _this6.loadMoreButton.prop('disabled', _this6.photoProvider.getCurrentAlbum().getURL() === false);
          _this6.toggleLoadingClass(false);
        });
      }
    ...
    key: 'handlePhotosButtonClick',
    value: function handlePhotosButtonClick() {
      this.switchToPanel('photos');
    }
  }, {
    key: 'handleAlbumsButtonClick',
    value: function handleAlbumsButtonClick() {
      var _this7 = this;
      ...
      this.photoProvider.getAlbums().then(function (albums) {
        _this7.renderAlbums(albums);
      }).then(function () {
        _this7.toggleLoadingClass(false);
      });
    }
  }, {
    key: 'handleAlbumClick',
    value: function handleAlbumClick(e) {
      var albumTn = (0, _jquery2['default'])(e.currentTarget),
    ...
    value: function handlePhotoClick(e) {
      ...
      var photo = (0, _jquery2['default'])(e.currentTarget),
    ...
    key: 'handlePhotoDoubleClick',
    value: function handlePhotoDoubleClick(e) {
      var photo = (0, _jquery2['default'])(e.currentTarget),
          photoId = photo.data('photo-id');
    ...
    value: function handleLoadMoreButtonClick(e) {
      e.preventDefault();
      this.loadMore();
    }
  }, {
    key: 'handleCancelButtonClick',
    value: function handleCancelButtonClick(e) {
      e.preventDefault();
      this.clearSelected(this.photosPanel);
      this.close();
    }
    ...
    key: 'getSelectedPhotoURL',
    value: function getSelectedPhotoURL() {
      return '';
    }
  }, {
    key: 'getSelectedPhoto',
    value: function getSelectedPhoto() {
    ...
    key: 'hasSelectedPhoto',
    value: function hasSelectedPhoto() {
    ...
    value: function setupTemplate() {
      this.template = (0, _jquery2['default'])(this.template).html();
      this.content = (0, _jquery2['default'])(this.template);

      this.photosPanel = (0, _jquery2['default'])('.photos-panel', this.content);
      this.albumsPanel = (0, _jquery2['default'])('.albums-panel', this.content);

      this.photosButton = (0, _jquery2['default'])('.button-photos', this.content);
      this.albumsButton = (0, _jquery2['default'])('.button-albums', this.content);
      ...
      if (!this.photoProvider.supportsAlbums()) {
        ...
        (0, _jquery2['default'])('.button-container', this.content).remove();
      }

      this.loadMoreButton = (0, _jquery2['default'])('.button-load-more', this.content);
      this.cancelButton = (0, _jquery2['default'])('.button-cancel', this.content);
      this.submitButton = (0, _jquery2['default'])('.button-submit', this.content);
...
},{"./ContentProvider":100,"./Lightbox":103,"./PhotoProvider":106,"./util":108,"jquery":96}],106:[function(require,module,exports){
'use strict';
...
var PhotoProvider = (function () {
  function PhotoProvider(url, albumsurl) {
    _classCallCheck(this, PhotoProvider);
    ...
    this.addAlbum(new _Album2['default']('default', 'All Photos', this.url));
  }

  _createClass(PhotoProvider, [{
    key: 'init',
    value: function init() {
    ...
    key: 'numAlbumPhotos',
    value: function numAlbumPhotos() {
      return this.getCurrentAlbum().getPhotoIDs().length;
    }
  }, {
    key: 'addPhoto',
    value: function addPhoto(photoInstance) {
      if (photoInstance instanceof _Photo2['default']) {
    ...
    key: 'addAlbum',
    value: function addAlbum(albumInstance) {
      if (albumInstance instanceof _Album2['default']) {
    ...
    key: 'hasPhoto',
    value: function hasPhoto(photoId) {
      return this.photos[photoId] && this.photos[photoId] instanceof _Photo2['default'];
    }
    ...
    key: 'getCurrentAlbumPhotos',
    value: function getCurrentAlbumPhotos() {
      var photoIds = this.getCurrentAlbum().getPhotoIDs(),
          photos = {},
          l = photoIds.length;

      for (var i = 0; i < l; ++i) {
        var photo = this.photos[photoIds[i]] || false;
    ...
    value: function getPhotos() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (!_this.getCurrentAlbum().hasPhotos()) {

          _this.loadPhotos().then(resolve, reject);
        } else {

          resolve(_this.getCurrentAlbumPhotos());
        }
      });
    }
  }, {
    key: 'loadPhotos',
    value: function loadPhotos(url) {
      var _this2 = this;

      var apiURL = url || this.getURL();

      if (apiURL !== false) {

        return this.api(apiURL, this.getParameters()).then(function (results) {
          return _this2.processResults(results);
        }, _util.throwIt);
      }
    ...
    key: 'getCurrentAlbum',
    value: function getCurrentAlbum() {
    ...
    key: 'hasAlbum',
    value: function hasAlbum(albumId) {
      return this.albums[albumId] && this.albums[albumId] instanceof _Album2['default'];
    }
    ...
    */
    ...
    key: 'apiGetPhoto',
    value: function apiGetPhoto(photoId) {
      var _this3 = this;
      ...
      return this.api(this.apiGetPhotoURL(photoId), this.getParameters()).then(function (data) {
        photo = _this3.buildPhoto(data);
        _this3.addPhoto(photo);
        return photo;
      }, _util.throwIt);
    }
  }, {
    key: 'apiGetAlbum',
    value: function apiGetAlbum(albumId) {
    ...
    value: function processResults(results) {
      ...
      var photos = {}; // This current batch of photos will be returned.

      results.data.forEach(function (d) {
        var photo = _this4.buildPhoto(d);
        _this4.addPhoto(photo);
    ...
    */
    ...
    key: 'processAlbumData',
    value: function processAlbumData(data) {
      var _this5 = this;

      if (!this.albumsLoaded) {

        data.forEach(function (d) {
          _this5.addAlbum(_this5.buildAlbum(d));
        });
    ...
    */
    ...
    key: 'api',
    value: function api() /* url, parameters */{
      _util.debug.info('PhotoProvider.api: Please override this method in your subclasses');
    ...
    key: 'apiScrape',
    value: function apiScrape(url, parameters) {
      var _this6 = this;

      var data = arguments.length <= 2 || arguments[2] === undefined ? [] : arguments[2];

      return this.api(url, parameters).then(function (results) {
        var nextURL = _this6.getNextUrl(results);

        data.push.apply(data, _toConsumableArray(results.data));

        if (nextURL) {
          return _this6.apiScrape(nextURL, parameters, data);
        }
    ...
    key: 'apiGetPhotoURL',
    value: function apiGetPhotoURL(photoId) {
      _util.debug.info('PhotoProvider.apiGetPhotoURL: Please override this method in your subclasses');
      return '/' + photoId;
    }
  }, {
    key: 'apiGetAlbumURL',
    value: function apiGetAlbumURL(albumId) {
      _util.debug.info('PhotoProvider.apiGetAlbumURL: Please override this method in your subclasses');
      return '/' + albumId;
    }
  }, {
    key: 'getNextUrl',
    ...
    key: 'buildPhoto',
    value: function buildPhoto() /* data */{
      return new _Photo2['default']();
    }
  }, {
    key: 'buildAlbum',
    value: function buildAlbum() /* data */{
      return new _Album2['default']();
    }
  }]);
...
},{"./Album":99,"./Photo":104,"./util":108}],107:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, '__esModule', {
  value: true
});
exports['default'] = getFB;
...
function rejectSDK(error) {
  rejecters.forEach(function (reject) {
    reject(error);
  });
  resetVars();
}
...
function getFB() {
  ...
  var version = _ref$version === undefined ? 'v2.3' : _ref$version;

  return new Promise(function (resolve, reject) {
    resolvers.push(resolve);
    rejecters.push(reject);
    loadSDK({ appId: appId, version: version });
  });
}

module.exports = exports['default'];

},{"jquery":96}],108:[function(require,module,exports){
'use strict';
...
var debug = {
  ...
  log: function log() {
    ...
    debug.console.apply(debug, ['log'].concat(args));
  },
  ...
  info: function info() {
    ...
    debug.console.apply(debug, ['info'].concat(args));
  },
  ...
  warn: function warn() {
    ...
    debug.console.apply(debug, ['warn'].concat(args));
  },
  ...
  error: function error() {
    ...
    debug.console.apply(debug, ['error'].concat(args));
  }
};
exports.debug = debug;
